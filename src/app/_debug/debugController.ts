...
debugController.post('/createDebugGame', async (ctx) => {
  ...
  await playCardAsManna(player1.id, player1.hand[0]);
  await playCardAsManna(player1.id, player1.hand[1]);
  await playCardAsManna(player1.id, player1.hand[2]);
  await endTurn(gameId, player1.id);

  await playCardAsManna(player2.id, player2.hand[0]);
  await playCardAsManna(player2.id, player2.hand[1]);
  await playCardAsManna(player2.id, player2.hand[2]);
  await endTurn(gameId, player2.id);

  await playCard(gameId, player1.id, player1.hand[3], 2, 2);
  await endTurn(gameId, player1.id);

  await playCard(gameId, player2.id, player2.hand[3], 2, 3);
  await endTurn(gameId, player2.id);

  await attackCard(gameId, player1.id, player1.hand[3], player2.hand[3]);
